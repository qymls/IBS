...
<div id="app">
    ...
    <Card>
        ...
        <Row>
            <i-col span="3">
                ...
                <Poptip
                        ...
                        transfer
                        title="您确认删除这些信息吗?"
                        @on-ok="deleteSystemdictionarydetail">
        ...
        <Row justify="center" align="middle">
            <div style="margin-top:20px">
                <i-Table :columns="columns" :data="SystemdictionarydetailData" border max-height="650"
                         @on-selection-change="deleteRows"
                         @on-row-dblclick="updateModelShow">
                    <template slot-scope="{ row }" slot="types">
                        <strong>{{ gettypes(row)}}</strong>
                    </template>
                </i-Table>
            </div>
            ...
            <Modal title="添加信息" v-model="updateModel" class-name="vertical-center-modal" footer-hide draggable>
                <i-Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
                    ...
                    <Form-Item label="类别" prop="typesId">
                        <i-Select v-model="formValidate.typesId">
                            <i-Option v-for="item in systemdictionarytypes" :value="item.id">{{item.name}}</i-Option>
                        </i-Select>
                    </Form-Item>
                    <Form-Item>
    ...
    <script src="admin/systemdictionarydetail/systemdictionarydetail.js" type="module"></script>
</div>