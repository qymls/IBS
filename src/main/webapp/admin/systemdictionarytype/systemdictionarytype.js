    ...
    methods: {
            ...
            refs[name].validate((valid) => {
                if (valid) {
                    ...
                    if (this.formValidate.id) {/*修改*/
                        url = "Admin/Systemdictionarytype/update"
                        param.action = "update"/*传递这个参数是配合 @ModelAttribute注解使用的，只用于修改*/
                    } else {/*添加*/
                        var url = "Admin/Systemdictionarytype/save";
                        param.action = "save";
                    }
                    $.ajax({
                        type: "POST",
        ...
        getFirstMenuData(page, pageSize) {
            ...
            $.ajax({
                type: "POST",
                contentType: "application/x-www-form-urlencoded",
                url: "Admin/Systemdictionarytype/findAll",
                data: {
                ...
                success: function (result) {/*用了框架的*/
                    if (result.msg) {/*操作失败，无权限*/
                        ...
                        $page.SystemdictionarytypeData = result.content;
        ...
        deleteSystemdictionarytype() {
            var $page = this;
            var notice = this.$Notice;
            $.ajax({
                type: "POST",
                contentType: "application/x-www-form-urlencoded",
                url: "Admin/Systemdictionarytype/delete",
